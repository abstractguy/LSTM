...
for epoch in range(10000):
	hidden_layers, output_layers, output_errors = list(), list(), list()
	hidden_layers.append(np.zeros(16))
	...
	for time in range(4):
		hidden_error = (next_hidden_error.dot(hidden_synapses.T) + output_errors[-time - 1].dot(output_synapses.T)) * (hidden_layers[-time - 1] * (1 - hidden_layers[-time - 1]))
