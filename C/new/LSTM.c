...
LSTM_type *make_LSTM(unsigned int rows, unsigned int columns) {
  ...
  // Empty inputs (Xt_i):
  LSTM = LSTM_initialize_tensors(LSTM, 0, 1, zero, 1, columns, rows);

  // Empty outputs (Yt_k):
  LSTM = LSTM_initialize_tensors(LSTM, 1, 2, zero, 1, rows, columns * 2);

  LSTM = LSTM_initialize_tensors(LSTM, GATES_BEGIN, GATES_END, one, 2, columns, rows);

  LSTM = LSTM_initialize_tensors(LSTM, INPUT_WEIGHTS_BEGIN, INPUT_WEIGHTS_END, random_long_double, 1, rows, columns);

  LSTM = LSTM_initialize_tensors(LSTM, HIDDEN_WEIGHTS_BEGIN, HIDDEN_WEIGHTS_END, random_long_double, 1, rows, rows);

  LSTM = LSTM_initialize_tensors(LSTM, CELL_WEIGHTS_BEGIN, CELL_WEIGHTS_END, random_long_double, 1, rows, columns);

  LSTM = LSTM_initialize_tensors(LSTM, ERRORS_BEGIN, ERRORS_END, zero, 1, columns, rows);

  LSTM = LSTM_initialize_tensors(LSTM, INPUT_UPDATES_BEGIN, INPUT_UPDATES_END, zero, 1, rows, columns);
